...
#' @param stratas teksts (\code{character}) - strata variable name;
...
nonresponse_correction <- function(in_system = "in_isdavs",
  ...
  skaits_stratas <- dataset[, lapply(.SD, sum, na.rm = TRUE), keyby = stratas,
  ...
  dataset <- merge(dataset, skaits_stratas, by = c(stratas), all = TRUE)
